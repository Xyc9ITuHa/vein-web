    ...
    return (
        <>
            <section id="hero" className="relative">
                <div className="relative min-h-screen flex flex-col hero-container">
                    {/* Main content */}
                    <div className="flex-1 flex flex-col items-center justify-center text-center px-4 py-8 sm:px-6 lg:px-8">
                        <div className="absolute -z-70 inset-0 overflow-hidden bg-gradient-to-bl from-cream to-accent w-full">
                            <img src={ring_sm} id="ring" alt="small ring from flowers photo" className="absolute opacity-15 sepia brightness-200 translate-y-1/2 right-1 left-1 bottom-0" />
                            <img src={ring_md} id="ring" alt="medium ring from flowers photo" className="absolute opacity-12 sepia-90 brightness-175 translate-y-1/2 right-1 left-1 bottom-0" />
                            <img src={ring_lg} id="ring" alt="large ring from flowers photo" className="absolute opacity-9 sepia-80 brightness-150 translate-y-1/2 right-1 left-1 bottom-0" />
                            <img src={ring_xl} id="ring" alt="very large ring from flowers photo" className="absolute opacity-6 sepia-70 brightness-100 translate-y-1/2 right-1 left-1 bottom-0" />
                        </div>
